  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-gray-500 mb-8">Désolé, la famille de produits que vous recherchez n'existe pas ou a été déplacée.</p>
        <Link href="/produits" className="bg-merlin-black text-white px-8 py-4 rounded-full font-bold uppercase tracking-widest hover:bg-merlin-red transition-colors">
          Retour au catalogue
        </Link>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 pb-24">
        ...
        >
          <div className="relative group">
            ...
            <div className="absolute -bottom-6 -right-6 bg-merlin-green text-white p-8 rounded-3xl shadow-2xl max-w-[200px] z-10 hidden md:block">
              <p className="font-black text-2xl uppercase leading-tight">QUALITÉ GARANTIE</p>
              <p className="text-white/70 text-xs mt-2 uppercase tracking-widest font-bold">Standard Merlin</p>
            </div>
          </div>
        ...
        <div className="space-y-10">
          <div className="space-y-4">
            <span className="text-merlin-red font-black text-xs uppercase tracking-widest px-4 py-1 bg-red-50 rounded-full border border-red-100">Catalogue Merlin Kribi</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-6">
            <div className="flex items-start gap-4 p-4 rounded-2xl bg-merlin-gray/50 border border-gray-100">
              ...
              <div>
                <h4 className="font-bold uppercase text-sm">Disponibilité immédiate</h4>
                <p className="text-xs text-gray-500 mt-1">Stock permanent dans nos agences de Kribi.</p>
              </div>
            </div>
            <div className="flex items-start gap-4 p-4 rounded-2xl bg-merlin-gray/50 border border-gray-100">
              <Truck className="w-6 h-6 text-merlin-red shrink-0" />
              <div>
                <h4 className="font-bold uppercase text-sm">Livraison express</h4>
                <p className="text-xs text-gray-500 mt-1">Livraison sur votre chantier en moins de 24h.</p>
              </div>
            </div>
            <div className="flex items-start gap-4 p-4 rounded-2xl bg-merlin-gray/50 border border-gray-100">
              <ShieldCheck className="w-6 h-6 text-merlin-green shrink-0" />
              <div>
                <h4 className="font-bold uppercase text-sm">Certification Pro</h4>
                <p className="text-xs text-gray-500 mt-1">Matériaux conformes aux normes internationales.</p>
              </div>
            </div>
            <div className="flex items-start gap-4 p-4 rounded-2xl bg-merlin-gray/50 border border-gray-100">
              <ShoppingCart className="w-6 h-6 text-merlin-red shrink-0" />
              <div>
                <h4 className="font-bold uppercase text-sm">Prix compétitifs</h4>
                <p className="text-xs text-gray-500 mt-1">Le meilleur rapport qualité/prix du marché.</p>
              </div>
            </div>
          </div>
      ...
      <section className="bg-merlin-gray py-24 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-12">
            <h2 className="text-3xl font-black uppercase">Autres <span className="text-merlin-red">Catégories</span></h2>
            <Link href="/produits" className="text-merlin-green font-bold uppercase text-sm hover:underline">Voir tout le catalogue</Link>
          </div>
